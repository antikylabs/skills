...
export const TRIGGER_CASES: EvalCase[] = [
  {
    ...
    expectation: "opens anti-slop rather than eyeballing the tree",
    assert: (t) => {
    ...
    script: [
      ...
      tool("read_file", { path: `/skills/general/${SKILL}/reference/structure.md` }),
...

  {
    ...
    expectation: "opens anti-slop rather than reviewing the prose by eye",
    assert: (t) => {
      const opened = skillInvoked(t, SKILL);
      return {
        passed: opened,
    ...
    script: [
      tool("read_file", { path: `/skills/general/${SKILL}/SKILL.md` }),
      tool("read_file", { path: `/skills/general/${SKILL}/reference/prose.md` }),
