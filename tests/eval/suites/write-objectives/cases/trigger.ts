...
export const TRIGGER_CASES: EvalCase[] = [
  {
    ...
    script: [tool("read_file", { path: `/skills/general/${SKILL}/SKILL.md` }), text("Loaded the objectives skill.")],
    negativeControl: {
...

  {
    ...
    script: [tool("read_file", { path: `/skills/general/${SKILL}/SKILL.md` }), text("Planning the goals.")],
    negativeControl: {
      reason: "an agent that treats a planning task as a decision record",
      trace: {
        toolCalls: [{ name: "read_file", args: { path: `/skills/general/${SKILLS.adr}/SKILL.md` }, blocked: false }],
