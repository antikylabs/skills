...
export const JUDGEMENT_CASES: EvalCase[] = [
...

  {
    ...
    script: [
      tool("read_file", { path: `/skills/${SKILL}/reference/gut-check.md` }),
      tool("read_file", { path: SOUND }),
      text(
...

  {
    ...
    script: [
      tool("read_file", { path: `/skills/${SKILL}/reference/ladder.md` }),
...

  {
    ...
    script: [
      tool("read_file", { path: `/skills/${SKILL}/reference/talk-it-out.md` }),
...

  {
    ...
    assert: (t) => {
      const said = t.finalText ?? "";
      const handsOff = /general-write-adrs|suggest an ADR|ADR.*(suggest|propose)|should be recorded/i.test(said);
    ...
    script: [
      tool("read_file", { path: `/skills/${SKILL}/reference/grill-it.md` }),
      tool("read_file", { path: OVERBUILT }),
      text(
        "Config precedence is being decided here by implication, which is the finding that outlasts " +
          "the proposal. That belongs in a record — run general-write-adrs suggest on it. I am not " +
