...
remained for the bounded follow-up recorded in the
[deferred pipeline outcome](13-deferred-pipeline-follow-up.md). That work owned source refresh,
admission decisions, and at most six new pages; this audit did not pre-admit any candidate.

At this audit point, the [target queue](../TARGETS.md) was cut from a 79-URL discovery history to 29
exact primary-artifact URLs. Goal 80 and objective closeout later removed the published candidates
and rejected router, leaving 22 deferred sources. Rejected, harvested, duplicate, and no-retry
targets remain recoverable in these research records instead of remaining active queue entries.
...
The serialized intake consumed by Goal 80 is preserved through the
[frozen follow-up manifest](13-deferred-pipeline-follow-up.md#frozen-selection-manifest). The
reconciled state at the audit was:
