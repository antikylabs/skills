...
| --- | --- | --- |
| **The thinking is unresolved** | Contradicts itself, hedges the central claim, or describes two incompatible things as one | `general-engineering talk-it-out` |
| **The decision is undocumented** | Depends on a choice nobody recorded, and the file is quietly re-deciding it | `general-write-adrs suggest` |
| **The shape is wrong** | Mixes reference with tutorial, buries the goal under implementation, serves two readers at once | `general-write-docs classify` then `audit` |
| **The prose is the problem** | The claim is clear and the structure is right, but the sentences are long, passive, or full of unapproved vocabulary | `general-simplified-technical-english audit` |
