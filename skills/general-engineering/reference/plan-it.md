...
For anything objective-sized, **the plan document belongs to `general-write-objectives`**. That skill
owns the lifecycle, the numbering, and the goal contract. Do not reimplement them here.

The division:

| This command | `general-write-objectives create-plan` |
| --- | --- |
...
- if this is objective-sized: say so, and hand to `general-write-objectives create-plan` for the
  document.
