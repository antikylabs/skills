...
Exact paths, filename patterns, and the archive location belong to a project's own `repo-` skill. In
an Antiky repository that is `repo-write-objectives`.
...
The exact patterns are a project convention. Read its `repo-` skill before creating a file.
...
If the work requires a decision to change, that is an ADR. Use `general-write-adrs suggest` and say
so in the plan rather than planning around it.

A project's required reading list belongs to its `repo-` skill.
